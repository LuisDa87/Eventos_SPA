    ...
    render: async () => {
        ...
        const renderEventAction = (event) => {
            if (session && session.role === 'register') {
                ...
                if (isSubscribed) return `<span class="font-bold text-green-600">¡Inscrito!</span>`;
                if (isFull) return `<span class="font-bold text-red-600">¡Lleno!</span>`;
            ...
            return `<span class="text-sm font-bold ${event.attendees.length >= event.capacity ? 'text-red-500' : 'text-green-500'}">${event.attendees.length >= event.capacity ? '¡Lleno!' : `${event.capacity - event.attendees.length} cupos`}</span>`;
        };

        const view = `
            <div class="p-8"> 
                <div class="relative h-96 bg-gray-800 text-white flex items-center justify-center rounded-lg overflow-hidden">
                    <img src="https://images.unsplash.com/photo-1501281668745-f7f57925c3b4?q=80&w=2070&auto=format&fit=crop" class="absolute w-full h-full object-cover opacity-50" alt="[Imagen de un concierto]">
                    <div class="relative z-10 text-center p-4">
                        ...
                        <a href="#events" class="mt-8 inline-block bg-orange-500 hover:bg-orange-600 text-white font-bold py-3 px-8 rounded-lg transition duration-300">Explorar Eventos</a>
                    </div>
                </div>

                <div id="events" class="container mx-auto py-12">
